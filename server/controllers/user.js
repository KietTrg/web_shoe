...
const register = asyncHandler(async (req, res) => {
  ...
  if (user) throw new Error("User has existed");
    ...
    //   }
    // );
    // const html = `Xin vui lòng click vào link dưới đây để hoàn tất quá trình đăng kí. Link sẽ hết hạn sau 15 phút <a href=${process.env.URL_SERVER}/api/user/finalregister/${token}>click here</a>`;
    if (newUser) {
...
const finalRegister = asyncHandler(async (req, res) => {
  ...
  if (notActivedEmail) {
    notActivedEmail.email = Buffer.from(
      notActivedEmail?.email?.split("@")[0],
      "base64"
    ).toString();
...
//refresh token dung de cap moi access
//access dung de xac thuc nguoi dung va phan quyen nguoi dung
...
const getCurrent = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById({ _id })
    ...
    .populate("wishlist", "title thumb price");
  return res.status(200).json({
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  const html = `Xin vui lòng click vào link dưới đây để thay đổi mật khẩu của bạn. Link sẽ hết hạn sau 15 phút <a href=${process.env.CLIENT_URL}/reset-password/${resetToken}>click here</a>`;
  const data = {
    email,
    html,
...
const getUsers = asyncHandler(async (req, res) => {
  ...
  //filtering
  if (queries?.name)
    formatedQueries.name = { $regex: queries.name, $options: "i" }; //'i': ko phan biet hoa hay thuong
...
const updateCart = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  const { pid, quantity = 1, color, size, price, thumbnail, title } = req.body;
  if (!pid || !color || !size) throw new Error("Missing inputs");
  const user = await User.findById(_id).select("cart");
  const alreadyProduct = user.cart.find(
    (el) =>
      el.product.toString() === pid && el.color === color && el.size === size
  );
  if (alreadyProduct) {
...
const updateWishlist = asyncHandler(async (req, res) => {
  ...
  if (alreadyWishlist) {
    ...
    return res.json({
      success: response ? true : false,
      mes: response ? "Updated your wishlish" : "Failed to update",
    });
  } else {
    const response = await User.findByIdAndUpdate(
      _id,
